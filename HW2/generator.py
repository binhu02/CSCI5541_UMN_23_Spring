...
def generate_text(lm, prefix):
    sentence = prefix
    count = 0
    while True:
        cond1 = prefix[0]
        cond2 = prefix[1]
        next_word = lm.generate(text_seed=cond1+cond2)
        if next_word == '</s>' or count > 20:
            break
...
def main():
    ...
    if isTest:
        ...
        prefixes = [["i", "would"], ["it", "is"], ["i", "do"], ["that", "is"], ['how', 'about']]
        print('Start generating')
        for name in author_names:
            for prefix in prefixes:
                generated_text = generate_text(data[name]['lm'], prefix)
